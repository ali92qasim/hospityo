/**
 * Medicine categories index — import UI and background import polling.
 */

import confirmDialog from './confirm-dialog';

const STORAGE_KEYS = {
    importKey: 'medicineCategoryImportKey',
    statusUrl: 'medicineCategoryImportStatusUrl',
    indexUrl: 'medicineCategoryImportIndexUrl',
    expiry: 'medicineCategoryImportExpiry',
    doneFlag: 'medicineCategoryImportDone',
};
...
function showImportResult(type, html) {
    const resultDiv = document.getElementById('medicine-category-import-result');
    if (!resultDiv) {
        return;
    }
...
function hideProgressBanner() {
    const banner = document.getElementById('medicine-category-import-progress');
    if (banner) {
...
function handlePostReloadResult() {
    ...
    try {
        ...
        showImportResult('success', 'Medicine categories imported successfully.');
    }
}
...
function renderDoneResult(data) {
    ...
    const msg = '<strong>' + (data.created ?? 0) + '</strong> category(ies) created, '
...
function bindImportForm(root) {
    const importButton = root.querySelector('[data-medicine-category-import-trigger]');
    const fileInput = root.querySelector('[data-medicine-category-import-file]');
    const form = root.querySelector('[data-medicine-category-import-form]');
    ...
    fileInput.addEventListener('change', function () {
        ...
        confirmDialog({
            ...
            detail: 'New categories will be created. Existing categories with the same code will be updated. CSV, .xls, and .xlsx files are accepted.',
            confirmText: 'Import',
            cancelText: 'Cancel',
            variant: 'success',
...
document.addEventListener('DOMContentLoaded', function () {
    const root = document.getElementById('medicine-categories-index');
    if (!root) {
        return;
    }
