...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-lg shadow-sm">
        <div class="p-4 sm:p-6 border-b border-gray-200">
            <h3 class="text-lg font-semibold text-gray-800">New Manual Journal Entry</h3>
        ...
        <form action="{{ route('accounting.store-journal-entry') }}" method="POST" class="p-4 sm:p-6 space-y-6" id="journalEntryForm">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="description" id="description" value="{{ old('description') }}" required
                        class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-medical-blue focus:border-medical-blue text-sm"
                        placeholder="e.g. Salary payment for June 2026">
                </div>
            </div>
                    ...
                    <table class="w-full text-sm" id="linesTable">
                        ...
                        <tbody id="linesBody">
                            ...
                            <tr class="border-t border-gray-200">
                                ...
                                <td class="px-3 py-2">
                                    <input type="text" name="lines[0][narration]"
                                        class="w-full border-gray-300 rounded text-sm focus:ring-medical-blue focus:border-medical-blue"
                            ...
                            <tr class="border-t border-gray-200">
                                <td class="px-3 py-2">
                                    <select name="lines[1][account_id]" required
                                        class="w-full border-gray-300 rounded text-sm focus:ring-medical-blue focus:border-medical-blue">
                                        <option value="">Select Account</option>
                                        @foreach($accounts as $account)
                                            <option value="{{ $account->id }}">{{ $account->code }} — {{ $account->name }}</option>
                                ...
                                <td class="px-3 py-2">
                                    <input type="text" name="lines[1][narration]"
                                        class="w-full border-gray-300 rounded text-sm focus:ring-medical-blue focus:border-medical-blue"
                                        placeholder="Optional">
                                </td>
                                <td class="px-3 py-2">
                                    <input type="number" name="lines[1][debit]" step="0.01" min="0" value="0"
                                        class="w-full border-gray-300 rounded text-sm text-right focus:ring-medical-blue focus:border-medical-blue debit-input"
                                        oninput="updateTotals()">
                                </td>
                                <td class="px-3 py-2">
                                    <input type="number" name="lines[1][credit]" step="0.01" min="0" value="0"
                                        class="w-full border-gray-300 rounded text-sm text-right focus:ring-medical-blue focus:border-medical-blue credit-input"
                                        oninput="updateTotals()">
                                </td>
                                <td class="px-3 py-2 text-center">
                            ...
                            <tr>
                                <td colspan="5" class="px-3 py-1">
                                    <p class="text-xs text-center" id="balanceStatus">
                                        <span class="text-gray-400">Enter amounts above</span>
                                    </p>
                                </td>
                            </tr>
                        </tfoot>
            ...
            <div class="flex items-center gap-3 pt-2">
                <button type="submit" id="submitBtn" class="bg-medical-blue text-white px-6 py-2.5 rounded-lg hover:bg-blue-700 transition-colors text-sm">
...
<script>
var lineIndex = 2; // already have lines[0] and lines[1]
