...
@section('title', 'Patient Ledger')
@section('page-title', 'Patient Ledger')

@section('content')
<div class="bg-white rounded-lg shadow-sm">
    {{-- Patient Filter --}}
    <div class="p-4 sm:p-6 border-b border-gray-200">
        <form method="GET" action="{{ route('accounting.patient-ledger') }}" class="flex flex-col sm:flex-row gap-3 items-end">
                ...
                <select name="patient_id" id="patient_id"
                    ...
                    @foreach($patients as $patient)
                        <option value="{{ $patient->id }}" {{ $patientId == $patient->id ? 'selected' : '' }}>
                            {{ $patient->name }} ({{ $patient->patient_no ?? $patient->id }})
                        </option>
                    @endforeach
                </select>
            </div>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-gray-500">
                                <i class="fas fa-user text-4xl mb-4 text-gray-300"></i>
                                <p>No ledger entries found for this patient.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    @else
        <div class="px-6 py-12 text-center text-gray-500">
            <i class="fas fa-user text-4xl mb-4 text-gray-300"></i>
            <p>Select a patient to view their ledger</p>
        </div>
    @endif
</div>
@endsection