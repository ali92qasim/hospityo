...
<div class="max-w-4xl mx-auto">
    <form action="{{ route('ot.surgeries.store') }}" method="POST" data-conflict-url="{{ route('ot.check-conflicts') }}" data-surgery-id="" data-team-index="0">
        @csrf
        ...
        <div class="bg-white rounded-lg shadow-sm mb-6">
            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="patient_id" required class="w-full border-gray-300 rounded-lg text-sm focus:ring-medical-blue focus:border-medical-blue">
                        <option value="">Select Patient</option>
                        @foreach($patients as $patient)
                            <option value="{{ $patient->id }}" {{ old('patient_id') == $patient->id ? 'selected' : '' }}>{{ $patient->name }} — {{ $patient->patient_no }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="doctor_id" required class="w-full border-gray-300 rounded-lg text-sm focus:ring-medical-blue focus:border-medical-blue">
        ...
        <div class="bg-white rounded-lg shadow-sm mb-6">
            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="surgery_type" required class="w-full border-gray-300 rounded-lg text-sm focus:ring-medical-blue focus:border-medical-blue">
                        <option value="elective" {{ old('surgery_type', 'elective') == 'elective' ? 'selected' : '' }}>Elective</option>
                        <option value="emergency" {{ old('surgery_type') == 'emergency' ? 'selected' : '' }}>Emergency</option>
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Anesthesia Type</label>
                    <select name="anesthesia_type" class="w-full border-gray-300 rounded-lg text-sm focus:ring-medical-blue focus:border-medical-blue">
                        <option value="">Select</option>
                        @foreach(['general', 'local', 'spinal', 'epidural', 'sedation'] as $a)
                            <option value="{{ $a }}" {{ old('anesthesia_type') == $a ? 'selected' : '' }}>{{ ucfirst($a) }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Procedure Code</label>
                    <input type="text" name="procedure_code" value="{{ old('procedure_code') }}"
                        class="w-full border-gray-300 rounded-lg text-sm focus:ring-medical-blue focus:border-medical-blue"
                        placeholder="Optional ICD/CPT code">
                </div>
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Pre-Op Diagnosis</label>
                    <textarea name="pre_op_diagnosis" rows="2" class="w-full border-gray-300 rounded-lg text-sm focus:ring-medical-blue focus:border-medical-blue">{{ old('pre_op_diagnosis') }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm mb-6">
            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div>
                    ...
                    <input type="date" name="scheduled_date" value="{{ old('scheduled_date', $prefilledDate) }}" required
                        class="w-full border-gray-300 rounded-lg text-sm focus:ring-medical-blue focus:border-medical-blue">
                ...
                <div>
                    ...
                    <select name="operation_theatre_id" class="w-full border-gray-300 rounded-lg text-sm focus:ring-medical-blue focus:border-medical-blue">
                        <option value="">Select Theatre</option>
                        @foreach($theatres as $theatre)
                            <option value="{{ $theatre->id }}" {{ old('operation_theatre_id') == $theatre->id ? 'selected' : '' }}>{{ $theatre->name }} ({{ ucfirst($theatre->type) }})</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        {{-- Surgical Team (Optional) --}}
        <div class="bg-white rounded-lg shadow-sm mb-6">
            <div class="p-6 border-b border-gray-200">
                <h4 class="text-md font-medium text-gray-800"><i class="fas fa-user-md mr-2 text-medical-blue"></i>Surgical Team (Optional)</h4>
            </div>
            <div class="p-6" id="teamSection">
                <div class="space-y-3" id="teamRows">
                    {{-- No default row — user clicks "Add Team Member" to add --}}
                </div>
        ...
        <div class="flex justify-end space-x-4">
            <a href="{{ route('ot.surgeries.index') }}" class="px-6 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 text-sm">Cancel</a>
