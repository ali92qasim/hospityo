...
<aside data-landmark="ipd-episode-sidebar" class="lg:col-span-1 space-y-4">
    ...
    <div class="bg-white rounded-lg shadow-sm p-5">
        ...
        @endforelse
        <button type="button" onclick="showTab('care-team')" class="mt-4 w-full text-sm text-purple-700 hover:text-purple-900 font-medium">
            <i class="fas fa-user-plus mr-1"></i>Manage Care Team
        </button>
    </div>

    <div class="bg-white rounded-lg shadow-sm p-4 space-y-2">
        <button type="button" onclick="showTab('admission')" class="w-full text-left px-3 py-2 rounded-lg hover:bg-purple-50 text-sm font-medium text-gray-700">
            <i class="fas fa-bed mr-2 text-purple-600"></i>Admission
        </button>
        <button type="button" onclick="showTab('vitals')" class="w-full text-left px-3 py-2 rounded-lg hover:bg-purple-50 text-sm font-medium text-gray-700">
            <i class="fas fa-heartbeat mr-2 text-red-500"></i>Record Vitals
        </button>
        <button type="button" onclick="showTab('consultation')" class="w-full text-left px-3 py-2 rounded-lg hover:bg-purple-50 text-sm font-medium text-gray-700">
            <i class="fas fa-stethoscope mr-2 text-medical-blue"></i>Consultation
        </button>
        <button type="button" onclick="showTab('gpe-tab')" class="w-full text-left px-3 py-2 rounded-lg hover:bg-purple-50 text-sm font-medium text-gray-700">
            <i class="fas fa-notes-medical mr-2 text-indigo-600"></i>GPE Records
        </button>
        @if($workflowData['show_investigations'])
            <button type="button" onclick="showTab('tests')" class="w-full text-left px-3 py-2 rounded-lg hover:bg-purple-50 text-sm font-medium text-gray-700">
                <i class="fas fa-flask mr-2 text-teal-600"></i>Investigations
            </button>
        @endif
    </div>
</aside>
