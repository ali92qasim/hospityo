...
@section('content')
<div class="mb-6">
    <h1 class="text-2xl font-bold text-gray-800">Edit Bed</h1>
</div>
    ...
    <form method="POST" action="{{ route('beds.update', $bed) }}">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" id="bed_number" name="bed_number" value="{{ old('bed_number', $bed->bed_number) }}" 
            ...
            <div>
                ...
                <select id="bed_type" name="bed_type" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-medical-blue focus:border-transparent" required>
                    ...
                    <option value="general" {{ old('bed_type', $bed->bed_type) == 'general' ? 'selected' : '' }}>General</option>
                    <option value="private" {{ old('bed_type', $bed->bed_type) == 'private' ? 'selected' : '' }}>Private</option>
                    <option value="icu" {{ old('bed_type', $bed->bed_type) == 'icu' ? 'selected' : '' }}>ICU</option>
                    <option value="emergency" {{ old('bed_type', $bed->bed_type) == 'emergency' ? 'selected' : '' }}>Emergency</option>
                </select>
                @error('bed_type')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="number" id="daily_rate" name="daily_rate" value="{{ old('daily_rate', $bed->daily_rate) }}" min="0" step="0.01"
                       class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-medical-blue focus:border-transparent" required>
                @error('daily_rate')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
