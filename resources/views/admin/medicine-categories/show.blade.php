...
@section('title', 'View Medicine Category')

@section('content')
<div class="max-w-5xl">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">{{ $medicineCategory->name }}</h1>
    ...
    <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label class="block text-sm font-medium text-gray-500 mb-1">Category Code</label>
                <p class="font-mono text-lg bg-gray-100 px-3 py-2 rounded">{{ $medicineCategory->code }}</p>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-500 mb-1">Status</label>
                <p>
                    <span class="px-3 py-1 text-sm rounded-full {{ $medicineCategory->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                        {{ $medicineCategory->is_active ? 'Active' : 'Inactive' }}
                    </span>
                </p>
            </div>
            <div class="md:col-span-2">
                <label class="block text-sm font-medium text-gray-500 mb-1">Description</label>
                <p class="text-gray-700">{{ $medicineCategory->description ?? 'No description provided' }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-lg font-semibold text-gray-800">
                Associated Medicines 
                <span class="text-sm font-normal text-gray-500">({{ $medicineCategory->medicines->count() }})</span>
            </h2>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @foreach($medicineCategory->medicines as $medicine)
                        <tr>
                            <td class="px-4 py-3">
        ...
        @else
            <div class="text-center py-8 text-gray-500">
                ...
                <p>No medicines in this category yet</p>
            </div>
        @endif
    </div>
</div>
