...
@section('title', 'Create Imaging Order')
@section('page-title', 'Create Imaging Order')
@section('page-description', 'Order imaging studies for a patient')
...
<div class="bg-white rounded-lg shadow-sm p-6">
    ...
    <form action="{{ route('imaging.orders.store') }}" method="POST" id="order-form" data-item-fk="imaging_study_id">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="special_instructions" rows="2" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-medical-blue">{{ old('special_instructions') }}</textarea>
            </div>
        </div>
        ...
        <div class="mb-4">
            ...
            <div class="overflow-x-auto border border-gray-200 rounded-lg">
                <table class="w-full text-sm" id="items-table">
                    ...
                    <tbody id="items-body" data-row-index="{{ count(old('items', [[]])) }}">
                        ...
                        <tr class="item-row border-t border-gray-100">
                                    ...
                                    @foreach($imagingStudies->groupBy('category') as $category => $group)
                                        <optgroup label="{{ ucfirst($category ?: 'General') }}">
                                            @foreach($group as $inv)
                                                <option value="{{ $inv->id }}" {{ ($item['imaging_study_id'] ?? '') == $inv->id ? 'selected' : '' }}>
                            ...
                            <td class="px-4 py-2 text-center">
                                <button type="button" class="text-red-400 hover:text-red-600 remove-row-btn remove-btn" title="Remove" style="{{ count($oldItems) <= 1 ? 'display:none' : '' }}">
        ...
        <div class="flex justify-end space-x-3 mt-6">
            <a href="{{ route('imaging.orders.index') }}" class="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">Cancel</a>
            <button type="submit" class="bg-medical-blue text-white px-6 py-2 rounded-lg hover:bg-blue-700">Create Order</button>
        </div>
    </form>
</div>
